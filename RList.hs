...
lookup (One t:ts) i = if i < size t then lookupTree t i else lookup ts (i - size t)
...
lookupTree (Node w t1 t2) i = if i < w `div` 2 then lookupTree t1 i else lookupTree t2 (i - w `div` 2)
